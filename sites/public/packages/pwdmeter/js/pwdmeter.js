...
function chkPass(pwd) {
	...
	/* Modify overall score value based on usage vs requirements */
		...
		$("#nAlphaUCBonus").html(sAlphaUC); 
		$("#nAlphaLCBonus").html(sAlphaLC);
		$("#nNumberBonus").html(sNumber);
		$("#nSymbolBonus").html(sSymbol);
		$("#nMidCharBonus").html(sMidChar);
		...
		$("#nAlphasOnlyBonus").html(sAlphasOnly); 
		$("#nNumbersOnlyBonus").html(sNumbersOnly); 
		$("#nRepCharBonus").html(sRepChar); 
		$("#nConsecAlphaUCBonus").html(sConsecAlphaUC); 
		$("#nConsecAlphaLCBonus").html(sConsecAlphaLC); 
		$("#nConsecNumberBonus").html(sConsecNumber);
		$("#nSeqAlphaBonus").html(sSeqAlpha); 
		$("#nSeqNumberBonus").html(sSeqNumber); 
		$("#nSeqSymbolBonus").html(sSeqSymbol); 
		...
		for (var c=0; c < arrCharsLen; c++) {
			...
			if (arrChars[c] == parseInt(minVal + 1)) { 
				nReqChar++; 
				oImg.addClass("pass"); 
				console.log('#div_' + arrCharsIds[c] + '-pass');
				//oBonus.parent().css("pass"); 
				oBonus.parent().css("pass"); 
			}else if (arrChars[c] > parseInt(minVal + 1)) { 
				nReqChar++; 
				console.log('#div_' + arrCharsIds[c] + '-exceed');
				oImg.addClass("exceed"); 
				oBonus.parent().css("exceed"); 
			}else { 
				console.log('#div_' + arrCharsIds[c] + '-fail');
				oImg.addClass("fail"); 
				//oBonus.parent().css("fail"); 
				oBonus.parent().css("fail"); 
			}
		}
		...
		$("#nRequirementsBonus").html(sRequirements);
	...
	if(arrPwdLen <= 0){
			...
			for (var i in arrPassDivs) { $('#'+arrPassDivs[i]).addClass("pass"); }
			for (var i in arrFailDivs) { $('#'+arrFailDivs[i]).addClass("fail"); }
		}
}
...
function initPwdChk(restart) {
	...
	var arrFailDivs = ["div_nLength","div_nAlphaUC","div_nAlphaLC","div_nNumber","div_nSymbol","div_nMidChar","div_nRequirements"];
	for (var i in arrZeros) { $('#'+arrZeros[i]).html("0"); }
	for (var i in arrPassPars) { $('#'+arrPassPars[i]).parent().css("pass"); }
	for (var i in arrPassDivs) { $('#'+arrPassDivs[i]).addClass("pass"); }
	for (var i in arrFailPars) { $('#'+arrFailPars[i]).parent().css("fail"); }
	for (var i in arrFailDivs) { $('#'+arrFailDivs[i]).addClass("fail"); }
	$("#passwordPwd").value = "";
	$("#passwordTxt").value = "";
	$("#scorebar").css(
	...
	if (restart) {
		$("#passwordPwd").addClass("");
		$("#passwordTxt").addClass("hide");
		togPwdMask();
	}
}
